...
public class ListActivityTest extends
	...
	public void testShareButtonClick(){
		FeedItem feedItem = (FeedItem)getActivity().getListAdapter().getItem(0);
		Activity currentActivity = getActivity();
		solo.clickInList(0);
		solo.clickOnButton(currentActivity.getString(R.string.share_button_label));		
		Intent shareIntent = (Intent)solo.getCurrentActivity().getIntent().getExtras().get(Intent.EXTRA_INTENT);
		assertEquals(currentActivity.getString(R.string.share_message_type),shareIntent.getType());
		assertEquals(Intent.ACTION_SEND,shareIntent.getAction());
		assertEquals(feedItem.getTitle(),shareIntent.getStringExtra(Intent.EXTRA_SUBJECT));
		assertTrue(shareIntent.getStringExtra(Intent.EXTRA_TEXT).contains(currentActivity.getString(R.string.share_message)));
		assertTrue(shareIntent.getStringExtra(Intent.EXTRA_TEXT).contains(feedItem.getContentURL()));
	}
}
