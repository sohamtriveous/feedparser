...
//		new ShortenURL(LOGIN, APIKEY)
		...
		shareButton.setOnClickListener(new View.OnClickListener() {			
			...
			public void onClick(View v) {
				...
				try{
					shortenedURL = new ShortenURL(getString(R.string.bitly_login),getString(R.string.bitly_api_key)).getShortUrl(url);
				}
				catch(Exception ex){
					Log.w("", "Exception while shortening the URL "+ex);
					ex.printStackTrace();
				}
				intent.putExtra(android.content.Intent.EXTRA_TEXT, getString(R.string.share_message)+" "+shortenedURL);	
